...
#echo -e "${A}██${X}  ${A}downloading additional files"
	xurl=https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/$app/extras
	x=https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/$app/extras/extras.txt
...
# include display output: 
   ...
   mkdir -p /userdata/system/pro/.dep 2>/dev/null
      if [[ ( -e "$tput" && "$(wc -c "$tput" | awk '{print $1}')" < "444" ) || ( ! -e "$tput" ) ]]; then
         rm "$tput" 2>/dev/null
         wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "$tput" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/.dep/.tput
      fi
      if [[ ( -e "$libtinfo" && "$(wc -c "$libtinfo" | awk '{print $1}')" < "444" ) || ( ! -e "$libtinfo" ) ]]; then
         rm "$libtinfo" 2>/dev/null
         wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $libtinfo https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/.dep/.libtinfo.so.6
      fi
...
# include display output: 
#
url_tput=https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/.dep/tput
url_libtinfo=https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/.dep/libtinfo.so.6
	#
   tput=/userdata/system/pro/.dep/tput
   libtinfo=/userdata/system/pro/.dep/libtinfo.so.6
   mkdir -p /userdata/system/pro/.dep 2>/dev/null
...
cd /tmp/ 
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "/tmp/batocera.pro-loader.mp4" "https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/.dep/loader.mp4"
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O "/tmp/batocera.pro-$app.sh" "https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/$app/$app.sh"
