...
# prepare/preload
	echo -e "\n\n\nfixing nvidia ld.so.cache"
		...
		rm /usr/bin/preload 2>/dev/null
			wget -q --tries=30 -O /usr/bin/prepare "https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/prepare.sh"
	...
	echo -e "\n\n\nfixing lutris"
		...
		cd /opt
			...
			rm $(which lutris) 2>/dev/null
			  wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O /usr/bin/lutris https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/lutris.sh
	...
	echo -e "\n\n\nfixing faudio staging"
		...
		yes "Y" | pacman -S faudio
			cd /tmp/
				...
				link=https://github.com/DRLEdition19/batocera.drl/raw/main/steam/build/lib32-faudio-tkg-git.pkg.tar.zst
