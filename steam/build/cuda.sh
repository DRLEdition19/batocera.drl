...
function pre1() {
    ...
    if [[ ! -s "$ubp" ]] || [[ ! -s "$bp" ]] || [[ ! -s "/userdata/system/pro/steam/prepare.sh" ]]; then
      ...
      mkdir -p "$c/usr/bin" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$ubp" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/prepare.sh
      ...
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$env" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/env.sh
...
function pre2() {
    ...
    if [[ ! -s "$f" ]] || [[ ! -s "/userdata/system/.local/share/Conty/passwd" ]]; then
      mkdir -p "$c/etc" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$f" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/passwd.sh
    ...
    if [[ ! -s "$c/etc/group" ]] || [[ ! -s "/userdata/system/.local/share/Conty/group" ]]; then
      mkdir -p "$c/etc" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$f" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/group.sh
      dos2unix "$f" 2>/dev/null
      chmod 777 "$f" 2>/dev/null
...
function pre3() {
    if [[ ! -s "$c/usr/bin/lutris" ]]; then
      mkdir -p "$c/usr/bin/" 2>/dev/null
      mkdir -p "$c/bin/" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$c/usr/bin/lutris" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/lutris.sh
...
if [[ ! -s "${nvdir}/.nvidia-$v-downloaded" ]]; then
    ...
    if [[ $use_aria = 1 ]] && [[ ! -e /usr/bin/aria2c ]]; then
        if [[ -e /userdata/system/.local/share/Conty/aria2c ]]; then
            ...
            wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O /usr/bin/aria2c https://github.com/DRLEdition19/batocera.drl/raw/main/.dep/aria2c
            chmod 777 /usr/bin/aria2c 2>/dev/null
...
# get nvidia-smi 
    ...
    wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$nvsmi" https://raw.githubusercontent.com/DRLEdition19/batocera.drl/main/steam/build/nvidia-smi
